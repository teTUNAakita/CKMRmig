...
\title{Estimating contemporary migration rates of adults for iteroparous species using kinship pairs found between populations}
\author{
...
In conservation and wildlife management, estimating the current ecological information is essential for monitoring population levels and proposing management strategies. Contemporary migration rate, the recent rate of movement of individuals or gametes between pre-defined populations, is a critical component to determine the current degree of gene flow \cite[]{Crow:1970fk} and synchrony among the population \cite[]{Hastings_1993}. Therefore, estimation of contemporary migration rate provides information about the degree of genetic differentiation and demographic dependency, which can delineate evolutionarily significant units \cite[ESUs,][]{moritz1994defining} and identify appropriate management units \cite[]{Waples_2006_MolEcol, PALSBOLL200711}. In addition, the availability of migration rate can link to underlying population dynamics, allowing for a more precise and flexible evaluation of a management for many relevant topics including species range expansion/construction (REF), effective reserve network design (REF), and the invasive species control (REF). 

 \RED{KOKOMADE}

% 2: Mark recapture method can handle ecological scale but may miss the small migration rate

There are essentially two approaches for estimating the contemporary migration rate. The first is to use the mark-recapture (MR) method that directly estimates parameters of the population.
