...
Finally, we note some caveats in applying proposed method. Our theory for developing estimators assumes that kinships are detected without any error. There are many algorithms to detect kinship pairs from single nucleotide polymorphisms (SNPs) or short tandem repeats (STRs) (\RED{REF}), although a HS pair requires many more DNA markers than a PO pair. In addition, iteroprous species potentially has several kinship types such as halfsib-uncle/nephew or halfsib-cousin, which are expected to appear frequently and should be correctly distinguished from HS pairs. It is desirable to estimate in advance how many markers are needed for kinship detections in target populations associated with simulation of a pedigree reconstruction including distant kinships (e.g., \cite{Anderson_2020}). Our proposed estimators only detect the parental movements on a period between breeding seasons in given populations. 

%必要条件としては、個体数管理のため、細かすぎる時空かんスケールはだめ。大雑把なpopulationを仮定したい時に限る（空間的に細かいグリッドは困難？）、集団構造がspecifyされた後に実施される。繁殖タイミングの間隔の移動を統合したもの、子供の移動については何も言ってない。繁殖タイミングの間の移動しか捕捉できない。
