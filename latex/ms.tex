...
%conn & bravington , Trenkel & bravingtonも引用しないと
% MigEst by Wangとの比較も...もう一度よくWang 2014を読む！どうやって推定しているのか...
%% 今回提案した方法は、ゴーストポップがあっても大丈夫
%% MigEstは、親は動かず、子が移動することを想定している
%% N = Neを想定
%移動には２種類ある。子の移動と親の移動である。前者は花粉やラーバなど。connectivityは前者が大事かもで、どこまで遠く行くのかも重要。移動する生物の場合、後者も重要。

%contemporary migration rateはほとんどBaysAssを使っている人の用語

% 1: Estimating contemporary migration is important for both ecologically and evolutionally reasons

In conservation and wildlife management, estimating the current ecological information is essential for monitoring population levels and proposing management strategies. Contemporary migration rate, the recent rate of movement of individuals or gametes between pre-defined populations, is a critical component to determine the current degree of gene flow and synchrony among the population \cite[]{Waples_2006_MolEcol}. Therefore, estimation of contemporary migration rate provides information about the degree of genetic differentiation and demographic dependency, which can delineate evolutionarily significant units \cite[ESUs,][]{moritz1994defining} and identify appropriate management units \cite[]{PALSBOLL200711}. In addition, the availability of migration rate can link to underlying population dynamics, allowing for a more precise and flexible evaluation of a management for many relevant topics including stock assessment in fisheries \cite[]{methot2013stock}, species range expansion/construction (REF), effective reserve network design (REF), and the invasive species control (REF). 

% 2: Mark recapture method can handle ecological scale but may miss the small migration rate

There are essentially two approaches for estimating the contemporary migration rate. The first is to use the mark-recapture (MR) method that directly estimates migration rate along with other population parameters \cite[]{kery2011bayesian, Thorson_2021}. The rational is that, for example, conventional tags provide release and recovery location at known release and recovery dates, generating a movement fraction matrix among strata per given time interval. While this direct method is relatively easy to interpret, it is generally suffering from several sources of uncertainties, e.g., reporting rate and tagging-involved mortality (REF). In addition, 

% 様々な不確実性だけでなく、応用の範囲が限られている


Direct estimates of migration parameters can be obtained using purely ecological approaches such as mark-release–recapture methods (MRR), but they have the inconvenience of being time consuming and impractical for the study of large and/or spatially extended metapop- ulations. 


 \RED{KOKOMADE}

% 3: Population genetics method can handle evolutionally scale but may miss the large migration rate
% 4: CKMR can cover the range that Mark recapture can handle with more realistic situations
% 5: Wang 2014 can handle ecological scale with realistic situations but limited to offspring movement
% 6: In this paper, ~

There are essentially two approaches for estimating the migrant number. The first is to use the mark-recapture method estimate it directly from the population dynamics modeling with survey data. The second is to ...
...
%Caveat: selectivity, family correlated migration、skip spawning, 
