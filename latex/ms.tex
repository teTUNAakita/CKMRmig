...
\date{Manuscript Intended for \emph{Molecular Ecology and Resources}, \today}
\maketitle

\begin{abstract}
This study develops estimators of the contemporary migration number and rate of adults for iteroparous species between two populations. The proposed estimators are based on both known half-sibling (HS) and/or parent-offspring (PO) relationships observed between the populations across the breeding seasons. The rationale is that such HS and PO pairs have information as to how often parental movements during the breeding interval are realized. Our method allows for variance of the average number of offspring per parent within and between populations. In addition, coupled with PO pairs found within population, the estimators can be obtained only from genetic data. Roughly speaking, a sample size of square root of population size is required for obtaining meaningful information on migration. We also evaluate the detailed performance of the estimators by running an individual-based model, providing a guidance for sample sizes to ensure the required accuracy and precision. Furthermore, in the face of the current situation where there is little effective method for estimating adult movement especially when populations cannot be genetically distinct, we discuss the usefulness of this proposed sibship assignment method in terms of conservation biology and wildlife management.  
\\
\end{abstract}

\textbf{Key words:} dispersion; sibship assignment; reproductive potential; close-kin mark-recapture\\
...
The author thanks N. Shinohara, N. Suzuki and Y. Tsukahara for fruitful discussions. This work was supported by JSPS KAKENHI Grant Number 19K06862 and 20H03012.
