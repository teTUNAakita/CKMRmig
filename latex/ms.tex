...
\title{Estimating migration rates for iteroparous species using kinship pairs found between populations\protect\thanks{Running head line: Estimating migration rates using kinship pairs}}
...
In this section, we derivate the estimators of migrant number and migration rates using HS pairs or PO pairs found between two populations. While the estimators can hold under complex assumptions for reproduction theoretical foundation for estimating
このセクションでは、migrantやmigration rateの推定量を導出する。導出された推定量は、付録にある通りフレキシブルな仮定のもとでも成立するが、ここではシンプルな状況を例に出して説明する。The main symbols used in the current paper are summarized in Table \ref{symbols}.
